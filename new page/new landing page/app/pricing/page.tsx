    ...
    <main className="relative min-h-screen bg-[#050505] text-[#f2ede6] overflow-x-hidden">
      ...
      <section className="relative min-h-screen flex items-center justify-center pt-20 overflow-hidden border-b border-[#1e1e1e]">
        ...
        >
          ...
          <motion.p variants={fadeInUp} className="text-base text-[#9a9a9a] leading-relaxed max-w-2xl mx-auto mb-8">
          ...
          <motion.div variants={fadeInUp} className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <a href="/signup" className="group inline-flex items-center gap-3 border border-[#1e1e1e] text-[#f2ede6] font-mono text-sm tracking-widest px-6 py-4 hover:border-[#2196f3]/40 transition-colors">
              START FREE TRIAL
            </a>
          </motion.div>
        </motion.div>
      ...
      <section id="plans" ref={ref} className="relative border-b border-[#1e1e1e] bg-[#080808]">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
              ...
              >
                ...
                <div className="p-8 border-b border-[#1e1e1e]">
                  <div className="flex items-start justify-between mb-4">
                    <span className="font-mono text-[9px] text-[#7a7a7a]">{p.id}</span>
                    {p.highlight && (
                      <span className="font-mono text-[9px] tracking-widest border border-[#2196f3]/40 text-[#2196f3] px-2 py-1">
                        POPULAR
                      </span>
                  ...
                  <p className="font-mono text-[10px] text-[#7a7a7a] tracking-wider">{p.tagline}</p>
                </div>
                ...
                <div className="p-8 border-b border-[#1e1e1e]">
                  {p.price.mo !== null ? (
                    <div className="flex items-baseline gap-2">
                      ...
                      <span className="font-mono text-[10px] text-[#7a7a7a]">/MONTH</span>
                    </div>
                  ) : (
                ...
                <ul className="p-8 space-y-3 border-b border-[#1e1e1e]">
                  {p.features.map(f => (
                    <li key={f} className="flex items-start gap-3">
                      ...
                      <span className="font-mono text-[11px] text-[#9a9a9a]">{f}</span>
                    </li>
                  ))}
                </ul>
                  ...
                  <a
                    ...
                    className={`w-full flex items-center justify-between font-mono text-[11px] tracking-widest px-5 py-4 transition-colors group ${
                      p.highlight
                        ...
                        : 'border border-[#2e2e2e] text-[#9a9a9a] hover:border-[#2196f3]/40 hover:text-[#2196f3]'
                    }`}
                  >
                    {p.cta}
          ...
          <p className="py-5 text-center font-mono text-[10px] text-[#7a7a7a]">
            ...
            {PRICING_FAQS.map((faq, i) => (
              <div key={i} className="transition-all duration-500">
                ...
                {active === i && (
                  ...
                  >
                    <p className="font-mono text-xs lg:text-sm text-[#9a9a9a] leading-relaxed max-w-3xl">
                      {faq.a}
                    </p>
                  </motion.div>
                )}
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-[#1e1e1e]">
                  ...
                  <th className="text-center py-4 px-6 font-mono font-semibold text-[#9a9a9a] text-xs lg:text-sm">HIRED SDR</th>
                  <th className="text-center py-4 px-6 font-mono font-semibold text-[#9a9a9a] text-xs lg:text-sm">OTHER AI</th>
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  <tr key={idx} className="border-b border-[#1e1e1e] hover:bg-[#0a0a0a] transition-colors">
                    ...
                    <td className="py-4 px-6 text-center text-[#9a9a9a] text-xs lg:text-sm">{row.sdr}</td>
                    <td className="py-4 px-6 text-center text-[#9a9a9a] text-xs lg:text-sm">{row.other}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section className="py-20 px-6 lg:px-12 text-center border-b border-[#1e1e1e]">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="max-w-2xl mx-auto">
          ...
          <p className="text-[#9a9a9a] mb-8">All plans include a 3-day free trial. No credit card required. Cancel anytime.</p>
