  ...
  Object.entries(leadsBySegment).forEach(([segmentId, leadIds]) => {
    ...
    while (leadsRemaining.length > 0) {
      ...
      for (let i = 0; i < dailyLimit && leadsRemaining.length > 0; i++) {
        ...
        let scheduledTime = new Date(currentLeadTime.getTime() + (jitterMinutes * 60 * 1000));

        if (leadTimezones && leadTimezones[leadId]) {
          try {
            const tz = leadTimezones[leadId];
            
            // Calculate timezone offset difference
            // We want the local hour in the lead's timezone to match currentLeadTime's local hour
            const formatterServer = new Intl.DateTimeFormat('en-US', {
              year: 'numeric', month: '2-digit', day: '2-digit',
              hour: '2-digit', minute: '2-digit', second: '2-digit',
              hour12: false
            });
            const formatterLead = new Intl.DateTimeFormat('en-US', {
              timeZone: tz,
              year: 'numeric', month: '2-digit', day: '2-digit',
              hour: '2-digit', minute: '2-digit', second: '2-digit',
              hour12: false
            ...
            // Hack to get the hour diff: Parse the formatted strings back to Dates
            // This gives us the difference between what the clock says in both places
            const serverDateStr = formatterServer.format(scheduledTime).replace(/, /g, ' ');
            const leadDateStr = formatterLead.format(scheduledTime).replace(/, /g, ' ');

            // Parse DD/MM/YYYY hh:mm:ss format generated by en-US
            const serverMs = new Date(serverDateStr).getTime();
            const leadMs = new Date(leadDateStr).getTime();
            
            if (!isNaN(serverMs) && !isNaN(leadMs)) {
              const diffMs = serverMs - leadMs;
              scheduledTime = new Date(scheduledTime.getTime() + diffMs);
            }
          } catch (e) {
