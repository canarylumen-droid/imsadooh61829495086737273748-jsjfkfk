...
export async function detectCompetitorMention(message: string): Promise<CompetitorMentionResult> {
  ...
  if (!detectedCompetitor) {
    const aiDetection = await generateReply(
      ...
      message,
      { model: "gpt-4o-mini", jsonMode: true, nga1Enforced: true }
    ).catch(() => null);
...
Do NOT mention you are an AI.`;
  ...
  const reply = await generateReply("You are an expert sales representative.", prompt, { model: "gpt-4o", nga1Enforced: true });
