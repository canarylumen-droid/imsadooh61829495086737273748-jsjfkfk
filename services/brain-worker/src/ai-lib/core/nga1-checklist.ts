/**
 * AUDNIX NGA-1 EXECUTION CHECKLIST
 * This is the "law" for the Audnix AI. Every AI run must satisfy these items.
 */
export const NGA1_CHECKLIST = `
### AUDNIX NGA-1 EXECUTION CHECKLIST — MANDATORY (THE LAW)

#### CONTEXT & MEMORY
- **Ingest all columns**: Map every field (City -> timezone, fund -> segment, role -> ICP). Never drop data.
...
- **Timezone-aware schedule**: Use BullMQ + Redis. Queue job for best local time per lead. Calculate heat time in seconds.
...
- **90-day unsupervised**: Must run 90+ days without human click, handling OOO, bounces, objections, no-shows.
`;

/**
 * Appends the NGA1 checklist to a system prompt.
 */
export function wrapWithNGA1(systemPrompt: string): string {
  return `${systemPrompt}\n\n${NGA1_CHECKLIST}`;
}
