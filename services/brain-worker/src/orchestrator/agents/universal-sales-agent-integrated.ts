...
import type { Lead as SchemaLead } from "@audnix/shared";
import {
  ...
  planProceduralMemory,
  gatherCompetitorIntelligence
} from "./universal-sales-agent.js";
import { calculateLeadScore } from "../../ai-lib/core/lead-management.js";
import {
  detectLeadIntent,
  suggestSmartReply,
  predictDealAmount,
...
} from "@services/brain-worker/src/ai-lib/context/lead-intelligence.js";
import {
  generateAutonomousObjectionResponse,
  recordObjectionLearning
} from "@services/brain-worker/src/ai-lib/analyzers/autonomous-objection-responder.js";
import {
  generateFollowRequest,
  shouldAskForFollow,
  handleFollowResponse
} from "@services/brain-worker/src/ai-lib/follow-request-handler.js";
...
function convertToSchemaLead(lead: LeadWithProfile): SchemaLead {
  return {
    id: lead.id,
    userId: lead.userId,
...
export async function generateContextAwareMessage(
  ...
  const baseResult = await generateOptimizedMessage(enhancedLead, brandContext, testimonials, stage);
  
  let enhancedMessage = baseResult.body;
  let enhancedSubject = baseResult.subject;
  ...
  return {
    ...
    explanation: `Targeting ${lead.firstName} at ${lead.company}. High value deal detected ($${prediction.predictedAmount}).`,
    autonomousClosing: `Generated via Universal Agent Integrated`,
    reasoning: {
      score,
      intent: intent.intentLevel,
      buyerStage: stage,
...
export const universalSalesAgent = {
  generateContextAwareMessage,
  handleLeadResponseWithLearning,
  autoGenerateFollowUp,
  evaluateNextBestAction: async (leadId: string, summary: string) => {
    // Adapter for orchestrator logic
    console.log(`🧠 Evaluating next best action for lead ${leadId}: ${summary}`);
  }
};
