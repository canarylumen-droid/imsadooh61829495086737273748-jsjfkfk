 ...
 * Wipes ALL user data: leads, mailboxes (integrations), campaigns, messages,
 * follow-up queue, and notifications. Keeps the user account itself intact.
 * 
 * Requires: { confirm: "CONFIRM_CLEAR_ALL_DATA" }
 */
router.post("/clear-data", async (req: express.Request, res: express.Response): Promise<void> => {
  try {
    ...
    if (confirm !== "CONFIRM_CLEAR_ALL_DATA") {
      res.status(400).json({
        error: "Confirmation required",
        message: "Send { confirm: 'CONFIRM_CLEAR_ALL_DATA' } to proceed",
        tables: ["integrations","leads","messages","outreach_campaigns","campaign_leads","follow_up_queue","notifications","bounce_tracker"]
      });
      return;
    }

    console.log("[ADMIN] ⚠️ CLEARING ALL DATA — leads, mailboxes, campaigns, messages...");

    // Order matters: delete child tables before parents to avoid FK violations
    await db.execute(sql`DELETE FROM follow_up_queue`);
    await db.execute(sql`DELETE FROM bounce_tracker`);
    await db.execute(sql`DELETE FROM campaign_emails`);
    await db.execute(sql`DELETE FROM campaign_leads`);
    await db.execute(sql`DELETE FROM outreach_campaigns`);
    ...
    console.log("[ADMIN] ✅ All data cleared. User accounts preserved.");

    res.json({
      success: true,
      message: "All leads, mailboxes, campaigns, and messages have been deleted. User accounts are intact.",
      cleared: ["follow_up_queue","bounce_tracker","campaign_emails","campaign_leads","outreach_campaigns","messages","notifications","leads","integrations"]
    });
  } catch (error: any) {
