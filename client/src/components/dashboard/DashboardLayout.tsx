  ...
  return (
    <RealtimeProvider userId={user?.id}>
      <TooltipProvider delayDuration={400}>
                          ...
                          {!isAutonomousMode ? (
                            ...
                            aiActions.slice(0, 3).map((log, i) => (
                              <motion.div 
                                ...
                                className="flex flex-col gap-1 p-2 rounded-xl bg-muted/20 border border-border/20 hover:bg-muted/30 transition-colors"
                              >
        ...
        {/* Main Content Area */}
        <div className="flex-1 flex flex-col min-w-0 bg-background relative z-10 transition-all duration-500 overflow-y-auto">
          {/* Top Header */}
          ...
          <div className="relative z-10 flex flex-1 min-h-0">
            <main className="flex-1 bg-background relative">
              ...
              <div className={cn("mx-auto w-full", !fullHeight && "max-w-7xl p-2 sm:p-4 lg:p-6", fullHeight && "h-full")}>
                {children}
              </div>
            </main>
          </div>
