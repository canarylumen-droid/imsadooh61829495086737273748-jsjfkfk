...
export default function CloserEngineLive() {
  ...
  const CopyButton = ({ text, label }: { text: string; label: string }) => (
    <button
      ...
      className="flex-shrink-0 p-2 rounded-xl bg-primary/10 hover:bg-primary/20 transition-all active:scale-90 group cursor-pointer"
    >
  ...
  const CollapsibleSection = ({
    ...
    return (
      <motion.div
        ...
        className={`rounded-2xl border transition-all duration-500 ${theme.border} ${theme.bg} overflow-hidden`}
      >
        <button
        ...
        >
          <div className="flex items-center gap-4">
            <div className={`p-2 rounded-xl bg-card border border-border/10 group-hover:scale-110 transition-transform`}>
            ...
            <div className="text-left">
              ...
              <span className="text-sm font-bold text-foreground uppercase tracking-tight">{title}</span>
            </div>
          </div>
          {isExpanded ? (
          ...
          {isExpanded && (
            ...
            >
              <div className="px-4 pb-4 mt-1">
                <div className="p-4 rounded-xl bg-background/50 border border-border flex items-start gap-4 shadow-inner">
                  <p className="text-xs text-foreground flex-1 leading-relaxed font-semibold tracking-tight">{content}</p>
  ...
  return (
    <PageWrapper className="max-w-7xl mx-auto selection:bg-primary selection:text-black space-y-8">
      {/* Header Section */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-primary/20 bg-primary/5 mb-2">
            ...
            <span className="text-[9px] font-bold uppercase tracking-wider text-primary">Live Intelligence Active</span>
          </div>
          <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-foreground to-foreground/70 bg-clip-text text-transparent">
            Closer Engine Live
          </h1>
          <p className="text-muted-foreground mt-1 text-sm font-medium">
            Input prospect resistance. Receive deterministic closing strategies.
          </p>
        </div>
      </div>
      ...
      <ResponsiveGrid className="grid-cols-1 lg:grid-cols-5 gap-8 items-start">
        <motion.div
          initial={{ opacity: 0, x: -20 }}
          animate={{ opacity: 1, x: 0 }}
          className="lg:col-span-2 space-y-6"
        >
          <Card className="p-6 rounded-2xl border-border/40 bg-card/40 backdrop-blur-xl space-y-6 relative overflow-hidden group shadow-xl">
            <div className="absolute inset-0 bg-grid opacity-[0.03] pointer-events-none" />

            <div className="space-y-1 relative z-10">
              <h3 className="text-[10px] font-bold uppercase tracking-wider text-primary">Input Vector</h3>
              <h4 className="text-lg font-bold text-foreground uppercase tracking-tight">Intercept Objection</h4>
            </div>
            ...
            <div className="relative z-10">
              <Textarea
                ...
                className="min-h-[180px] rounded-xl bg-background/50 border border-border/40 text-foreground placeholder:text-muted-foreground/30 focus:border-primary/50 text-sm font-medium leading-relaxed resize-none p-4 transition-all shadow-inner"
              />
            </div>
            ...
            <Button
              ...
              className="w-full h-12 rounded-xl bg-white text-black font-bold uppercase tracking-wider shadow-md hover:bg-neutral-100 transition-all text-xs"
            >
            ...
            <div className="flex items-center justify-center gap-2 text-[9px] font-bold uppercase tracking-wider text-muted-foreground/30 relative z-10">
          ...
          <AnimatePresence mode="wait">
            {analysis ? (
              <motion.div
                ...
                className="space-y-6"
              >
                <div className="p-4 rounded-xl border border-orange-500/20 bg-orange-500/[0.05] flex items-center justify-between mb-4 shadow-sm">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-orange-500/10 flex items-center justify-center border border-orange-500/20">
                      <AlertCircle className="w-5 h-5 text-orange-500" />
                    </div>
                    <div>
                      <span className="text-[9px] font-bold uppercase tracking-wider text-orange-500/40 block mb-0.5">Inferred Psychological Subtext</span>
                      <h3 className="text-md font-bold text-foreground uppercase tracking-tight">
                        {analysis.hiddenObjection || analysis.category}
                      </h3>
                    </div>
                  </div>
                  <div className="text-right">
                    <span className="text-[9px] font-bold uppercase tracking-wider text-muted-foreground/30 block mb-0.5">Confidence</span>
                    <span className="text-lg font-bold text-foreground tracking-tight">{analysis.confidence}%</span>
                  </div>
                </div>
                ...
                <Button
                  variant="ghost"
                  className="w-full h-11 rounded-xl text-[10px] font-bold uppercase tracking-wider text-muted-foreground/40 hover:text-foreground transition-all border border-border/10 mt-4"
                  onClick={() => {
                    setProspectText("");
                    setAnalysis(null);
                  }}
              ...
              <Card className="h-full flex flex-col items-center justify-center p-12 bg-card/40 backdrop-blur-xl rounded-2xl border border-border/10 text-center space-y-6 shadow-xl">
                <div className="w-16 h-16 rounded-full bg-muted/20 flex items-center justify-center border border-border/10 mb-2">
                  <Brain className="w-8 h-8 text-muted-foreground/30 animate-pulse" />
                </div>
                <div className="space-y-2">
                  <h3 className="text-xl font-bold text-foreground uppercase tracking-tight">Awaiting Signal.</h3>
                  <p className="text-muted-foreground font-medium text-sm max-w-xs mx-auto leading-tight">
