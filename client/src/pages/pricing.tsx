...
const PLANS = [
  {
    ...
    price: { mo: 0, yr: 0 },
    features: ["10,000 lead sync", "2 Connected Mailboxes", "100 AI Voice Minutes", "Basic Cadence", "Standard Support"],
  ...
  },
  {
    ...
    price: { mo: 49, yr: 39 },
    features: ["2,500 leads / month", "5 Connected Mailboxes", "250 AI Voice Minutes", "Autonomous Follow-ups", "Smart CRM Integration", "Performance Analytics", "Conversion Tracking"],
    cta: "START TRIAL",
    highlight: false,
  },
  {
    ...
    price: { mo: 99, yr: 79 },
    features: ["7,000 leads / month", "15 Connected Mailboxes", "1,000 AI Voice Minutes", "Deep Lead Insights", "Intent Recognition", "110+ Objection Mastery", "Strategic ROI Mapping", "Priority Support"],
    cta: "START TRIAL",
    highlight: true,
  },
  {
    ...
    features: ["Unlimited leads", "Unlimited Mailboxes", "Unlimited Voice Minutes", "Voice Cloning & Training", "Smart Auto-Tagging", "Drop-off & Churn Detection", "Dedicated Success Manager", "24/7 Priority Support"],
    cta: "CONTACT SALES",
    highlight: false,
  },
];
...
const PRICING_FAQS = [
  { q: "Can I change plans anytime?", a: "Yes. Upgrade or downgrade your plan anytime. Changes take effect on your next billing cycle. No penalties or long-term contracts." },
  { q: "What payment methods do you accept?", a: "We accept all major credit cards (Visa, Mastercard, Amex), wire transfers for enterprise accounts, and ACH payments. Invoicing available for annual plans." },
  { q: "Do you offer discounts for annual billing?", a: "Yes. All plans include 20% savings when you pay annually. Plus, enterprise customers get custom volume discounts." },
  { q: "What's included in the free trial?", a: "Your 3-day trial includes full access to all features: 10,000 lead syncs, AI voice minutes, integrations, and everything except multi-user seats which require upgrade." },
  { q: "Is there a limit on how many team members can use my account?", a: "Growth and Performance plans include 3 team seats. Enterprise includes unlimited seats. Add additional seats at $50/user/month on Growth and Performance." },
];
...
export default function PricingPage() {
  ...
  useSEO(
    "Pricing - Audnix AI | AI Sales Agent Plans Starting at $0",
    "Compare Audnix AI pricing plans. Start with a 3-day free trial. Growth ($49/mo), Performance ($99/mo), Enterprise (custom pricing). No hidden fees, cancel anytime."
  );
  ...
  return (
    <main className="min-h-screen bg-background text-[#f2ede6] overflow-x-hidden">
      ...
      <section className="relative min-h-screen flex items-center justify-center pt-20 overflow-hidden border-b border-[#1e1e1e]">
        <motion.div initial="initial" animate="animate" variants={staggerContainer} className="relative z-10 max-w-5xl mx-auto text-center px-6 lg:px-12">
          ...
          <motion.h1 variants={fadeInUp} className="font-display text-[clamp(3rem,12vw,7rem)] leading-[0.88] tracking-tight mt-6 mb-6 font-black">
            Scale Revenue,<br /><span className="text-[#2196f3] font-black">Not Costs</span>
          </motion.h1>
      ...
      {/* Pricing Plans */}
      <section id="plans" className="relative border-b border-[#1e1e1e] bg-[#080808]">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
          {/* Header */}
          <div className={`border-b border-[#1e1e1e] py-8 flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 transition-all duration-500 ${vis ? 'opacity-100' : 'opacity-0'}`}>
            <div>
            ...
            <div className="flex items-center gap-4">
              ...
              <button onClick={() => setAnnual(!annual)} className="relative w-12 h-6 bg-[#1e1e1e] border border-[#2e2e2e] flex items-center">
              ...
              {annual && <span className="font-mono text-[9px] tracking-widest border border-[#2196f3]/40 text-[#2196f3] px-2 py-1">SAVE_20%</span>}
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 border-b border-[#1e1e1e]">
            {PLANS.map((p, i) => (
              <div key={p.id} className={`border-r border-b border-[#1e1e1e] last:border-r-0 relative transition-all duration-500 ${p.id === '04' ? 'md:col-span-2 lg:col-span-3' : ''} ${p.highlight ? 'bg-[#0e0e0e]' : ''} ${vis ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`} style={{ transitionDelay: `${i * 80}ms` }}>
                {p.highlight && <div className="absolute top-0 left-0 right-0 h-px bg-[#2196f3]" />}
                <div className="p-8 border-b border-[#1e1e1e]">
                  <div className="flex items-start justify-between mb-4">
                    <span className="font-mono text-[9px] text-[#3a3a3a]">{p.id}</span>
                    {p.highlight && <span className="font-mono text-[9px] tracking-widest border border-[#2196f3]/40 text-[#2196f3] px-2 py-1">POPULAR</span>}
                  </div>
                ...
                <div className="p-8 border-b border-[#1e1e1e]">
                  <div className="flex items-baseline gap-2">
                    {p.price.mo !== null ? (
                      <>
                        <span className="font-display text-5xl lg:text-6xl text-[#f2ede6]">${annual ? p.price.yr : p.price.mo}</span>
                        <span className="font-mono text-[10px] text-[#3a3a3a]">/MONTH</span>
                      </>
                    ) : (
                      <span className="font-display text-5xl lg:text-6xl text-[#f2ede6]">CUSTOM</span>
                    )}
                  </div>
                </div>
                ...
                <div className="p-8">
                  <Link href="/auth">
                    <Button className={`w-full flex items-center justify-between font-mono text-[11px] tracking-widest px-5 py-4 transition-colors group h-auto rounded-none ${p.highlight ? 'bg-[#2196f3] text-[#050505] hover:bg-[#60a5fa] font-semibold' : 'border border-[#2e2e2e] text-[#5a5a5a] hover:border-[#2196f3]/40 hover:text-[#2196f3] bg-transparent'}`}>
                      {p.cta}
          ...
          <p className="py-5 text-center font-mono text-[10px] text-[#3a3a3a]">ALL PLANS INCLUDE E2E ENCRYPTION \u00b7 SOC 2 COMPLIANCE \u00b7 99.97% UPTIME SLA</p>
        </div>
      </section>
      ...
      <section className="relative border-b border-[#1e1e1e]">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
          ...
          <p className="py-8 text-center font-mono text-[10px] text-[#3a3a3a]">
            Still have questions? <a href="#" className="text-[#2196f3] hover:text-[#60a5fa] transition-colors">CONTACT OUR TEAM</a>
          </p>
        </div>
      </section>
      ...
      <section className="sr-only opacity-0 pointer-events-none h-0 overflow-hidden" aria-hidden="true">
        <h2>Audnix Pricing & Sales Automation Keywords</h2>
        <p>
          audnix pricing, ai sales agent cost, affordable ai sdr, cold email software pricing, ai outreach pricing,
          best value ai sales tool, sales automation cost, ai bdr pricing, sdr replacement cost, email automation pricing,
          audnix growth plan, audnix performance plan, audnix enterprise, ai sales platform pricing 2026,
          audnix ai, nleanya treasure, uchendu fortune, autonomous sales rep, alternative to instantly,
          alternative to smartlead, ai lead generation, b2b sales automation, best ai sales software 2026
        </p>
      </section>
